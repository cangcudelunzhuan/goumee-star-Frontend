const mutations = {
  ...
  selectAnchor(state, { rangeIndex, anchorIndex }) {
    let flag = state.matchedInfoList[state.currentChooseAnchorIndex]
      .cost_range_list[rangeIndex].star_list[anchorIndex].isSelect
    state.matchedInfoList[state.currentChooseAnchorIndex].cost_range_list[rangeIndex].star_list[anchorIndex].isSelect = flag ? 0 : 1
  ...
  updateDisplayNum(state, { rangeIndex, anchorIndex, number }) {
    let _matchedInfoList = JSON.parse(JSON.stringify(state.matchedInfoList))
    _matchedInfoList[state.currentChooseAnchorIndex]
      .cost_range_list[rangeIndex].star_list[anchorIndex].display_num = number
  ...
  addStar(state, {star}) {
    ...
    state.matchedInfoList[state.currentChooseAnchorIndex].cost_range_list[rangeIndex].star_list.unshift(star)
  },
  // 删除候选集里的主播
  delStar(state, {rangeIndex, anchorIndex}) {
    state.matchedInfoList[state.currentChooseAnchorIndex].cost_range_list[rangeIndex].star_list.splice(anchorIndex, 1)
  },
  // 删除已确定的主播
