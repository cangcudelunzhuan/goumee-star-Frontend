...
<template>
  <section class="page-section page-section-star">
    <section style="background:#fff;padding:10px;margin-top:12px">
      <el-form :inline="true" :model="starQueryForm" size="small" class="query-star" label-position="right">
        <el-row>
          <el-form-item label="主播昵称">
            <!-- 带输入建议 :fetch-suggestions="querySearch" :trigger-on-focus="false" -->
            <el-input v-model="starQueryForm.star_name" placeholder="请输入主播昵称" @select="handleSelect" clearable @clear="handleStarNameClear">
            </el-input>
          </el-form-item>
      ...
      <el-form :inline="true" :model="starQueryForm" size="small" class="query-star-pv" label-position="right">
        <el-row>
          <el-form-item label="近15场PV">
            <el-select v-model="starQueryForm.mix_or_special" placeholder="请选择播放类型" clearable>
              <el-option v-for="item in mixSpeicalOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
            <el-input v-model="starQueryForm.min_pv" placeholder="请输入PV下限" type="number"></el-input>
            <span class="to-word">至</span>
            <el-input v-model="starQueryForm.max_pv" placeholder="请输入PV上限" type="number"></el-input>
          </el-form-item>
        </el-row>
      </el-form>
      <el-form :inline="true" :model="starQueryForm" size="small" class="query-star" label-position="right">
        <el-row>
          ...
          <el-form-item label="粉 丝 数">
            <el-select v-model="starQueryForm.fans_number" clearable placeholder="请选择">
              <el-option v-for="item in fansNumberOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-form-item>
        </el-row>
      </el-form>
      ...
      <el-row style="margin-top:14px;padding-left:10px">
        <el-button v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.view_kol)" type="primary" size="small" @click="addStarDialog">新增主播</el-button>
        <el-button v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.view_kol)" type="primary" size="small" @click="lotImportDialogVisible = true">批量导入</el-button>
        <el-button v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.del_kol)" type="primary" size="small" @click="showDelete" v-show="!notData">批量删除</el-button>
        <el-button v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.export_kol)" type="primary" size="small" @click="showExport" style="width:80px">导出</el-button>
        <el-button type="primary" size="small" @click="queryStarsubmit" style="width:80px">查询</el-button>
        ...
        <template v-if="isLotDelete">
          <el-checkbox style="margin-left:16px" @change="clickAllCheckbox" v-model="isCheckedAll">全选</el-checkbox>
          <!-- </el-checkbox-group> -->
    ...
    <!-- 批量导入弹窗 -->
    <el-dialog custom-class="dialog-min-width-import" title="批量导入" :visible.sync="lotImportDialogVisible" center append-to-body width="32%" class="lot-import-dialog" :close-on-click-modal="false"
      :close-on-press-escape="false">
      <el-row type="flex" justify="space-between">
        <el-upload :action='""' :http-request="uploadStarFile" :show-file-list="false">
          <el-button size="mini" type="primary" :loading="uploadLoading">批量导入主播基本信息</el-button>
        </el-upload>
        <el-upload :action='""' :http-request="uploadStarCostFile" :show-file-list="false">
          <el-button size="mini" type="primary" :loading="uploadCostLoading">批量导入主播成本报价</el-button>
        </el-upload>
      </el-row>
      <el-row type="flex" justify="space-between">
        <el-button size="mini" type="primary" @click="downloadModelxlsx">下载主播基本信息模板</el-button>
        <el-button size="mini" type="primary" @click="downloadStarCostModel">下载主播成本报价模板</el-button>
      </el-row>
    </el-dialog>
    <ul class="show-search-info" v-show="!notData">
      <li :class="{'is-marginLeft' : index%2 === 0, 'is-marginRight' : index%2 !== 0}" v-for="(item,index) in starQueryData" :key="index">
        <div class="card-head">
          <div class="card-head-left">
            <img :src="item.pic_url" alt="" @click="goStarDetail(item.star_name)">
            <span class="star-name" @click="goStarDetail(item.star_name)">{{item.star_name}}
              <el-button icon="el-icon-arrow-right" class="go-detail" @click="goStarDetail(item.star_name)"></el-button>
            </span>
          </div>
           <!-- v-if="isAdminUser" -->
          <template>
            <i class="card-edit" @click="openSaveDialog(item)" v-if="!(isLotDelete||isExport)"></i>
            <i class="card-delete" @click="openDeleteDialog(item.star_id)" v-if="!(isLotDelete||isExport)"></i>
          </template>
          <el-checkbox-group v-model="checkAllData" v-show="isLotDelete||isExport" @change="clickCheckBox">
        ...
        <div class="card-body">
          <div class="basic-info">
            ...
            <p>配合度：<el-rate disabled text-color="#ff9900" v-model="item.responsivity"></el-rate>
            </p>
            <p>粉丝数：{{item.fans_number}} 万</p>
            <p>手机号：{{item.star_mobile}}</p>
            <p>微信号：{{item.star_wechat}}</p>
          </div>
          <div class="card-body-middle" :class="[item.cate_click_num_items.length === 0 && item.display_type.length === 0 ? 'isRelativeLeft':'noRelativeLeft']">
            <p class="card-body-title">主播成本及报价/元</p>
            <p class="star-cost-info">混播成本：{{item.star_mix_cost === 0 ? '-' : item.star_mix_cost}}</p>
            <p class="star-cost-info">混播报价：{{item.star_mix_price === 0 ? '-' :
              item.star_mix_price}}</p>
            <p class="star-cost-info">专场成本：{{item.star_special_cost === 0 ? '-' :
              item.star_special_cost}}</p>
            <p class="star-cost-info">专场报价：{{item.star_special_price === 0 ? '-' :
              item.star_special_price}}</p>
          ...
          <div class="card-body-last" v-show="true">
            <p class="card-body-title" v-show="item.cate_click_num_items.length !== 0">最近30款产品点击数/次</p>
            <div class="card-body-last-des" v-show="item.cate_click_num_items.length !== 0">
              <p class="small-line-height" v-for="(items,ind) in showRecentClickNum15[index].cateInfo" :key="ind">{{items[0]}}：{{items[1]}}</p>
            </div>
            <p class="card-body-title" v-show="item.display_type.length !== 0">最近15场场均PV /万</p>
            <p class="small-line-height" v-if="item.display_type.length !== 0">{{item.display_type[0][0]}}：{{item.display_type[0][1]}}</p>
            <p class="small-line-height" v-if="item.display_type.length > 1">
              {{item.display_type[1][0]}}：{{item.display_type[1][1]}} </p>
          </div>
        </div>
        <div class="card-foot">
          <p>录入人：<span>{{getZeroNum(item.add_by)}}</span></p>
          <p>最终修改人：<span>{{getZeroNum(item.modified_by)}}</span></p>
          <el-button size="mini" type="primary" @click="openAddDisplayDialog(item)">新增场次</el-button>
    ...
    <!-- add-display-dialog -->
    <el-dialog title="新增场次" :visible.sync="addDisplayDialogVisible" @close="closeAddDisplayDialog" :close-on-press-escape="false" :close-on-click-modal="false" width="36%" class="add-display-form"
      custom-class="dialog-min-width">
      <displayForm :displayInfoForm="displayInfoForm" @displayFormRef="displayFormRef"></displayForm>
    ...
    <el-dialog title="主播信息" :visible.sync="saveStarDialogVisible" class="add-star-dialog" :close-on-click-modal="false" :close-on-press-escape="false" @closed="editDialogClose('starInfoForm')"
...
import { presellSelectOption, categoryOptions, salesPriceOptions, displayTypeOptions, fansNumberOptions, mixSpeicalOptions, clickTypeOptions } from '@/const/options'
...
export default {
  ...
  methods: {
        ...
        this.$confirm('确认删除主播?', '提示', {
          ...
          .then(() => {
            this.checkAllData.forEach(item => {
              this.delStarId.push(parseInt(item))
            })
            let starinfopass = {
    ...
    // 弹框关闭重置表单验证提示
    closeAddDisplayDialog(formName) {
        ...
        if (valid) {
          saveDisplay(this.displayInfoForm).then(response => {
            ...
            if (data.success) {
              ...
              this.$router.push({path: '/operation-center/display-management', query: { starName: this.displayInfoForm.star_name }})
            } else {
              this.$gmMessage(data.message, 'tip')
            }
          })
    ...
    querySearch(queryString, cb) {
      var queryStarSuggest = this.queryStarSuggest
      var results = queryString ? queryStarSuggest.filter(this.createFilter(queryString)) : queryStarSuggest
      cb(results)
    },
    // 输入建议
    ...
    getSalesPricePeriod(val) {
      for (var i = 0; i < this.salesPriceOptions.length; i++) {
        var option = this.salesPriceOptions[i]
        if (val === option.value) {
          return option.label
        }
      }
    ...
    showDisplay(scope) {
      ...
      this.$router.push({path: '/operation-center/display-management', query: { starName: scope.star_name }})
    },
    categoryFormate
  },
  mounted() {
