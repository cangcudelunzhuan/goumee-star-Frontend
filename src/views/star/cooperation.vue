 ...
 * @LastEditTime: 2019-08-06 16:21:43
...
<template>
  <div id="cooperation">
    <div class="condition">
      <div class="c-item">
        <span>搜索主播</span>
        <el-input placeholder="主播昵称/淘客ID" size="small" style="width: 170px;" v-model="condition.star_name"></el-input>
      </div>
      <div class="c-item">
        <span>粉丝数</span>
        <el-select v-model="condition.fans_number" placeholder="请选择" size="small" style="width: 160px;" >
      ...
      <div class="c-item cost">
        ...
        <el-select class="cost-type fl" v-model="condition.cost_mix_or_special" placeholder="请选择" size="small" style="width: 73px;">
          <el-option v-for="item in mixSpeicalOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
        <el-input class="cost-input fl" type="number" v-model="condition.min_cost" placeholder="输入成本" size="small" style="width: 86px;"></el-input>
        <em class="fl" style="margin: 0 5px;">-</em>
        <el-input class="fl" type="number" v-model="condition.max_cost" placeholder="输入成本" size="small" style="width: 86px;"></el-input>
      </div>
      <div class="c-item cost">
        <span class="fl">报价</span>
        <el-select class="cost-type fl" v-model="condition.price_mix_or_special" placeholder="请选择" size="small" style="width: 73px;">
          <el-option v-for="item in mixSpeicalOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
        <el-input class="cost-input fl" type="number" v-model="condition.min_price" placeholder="输入报价" size="small" style="width: 86px;"></el-input>
        <em class="fl" style="margin: 0 5px;">-</em>
        <el-input class="fl" type="number" v-model="condition.max_price" placeholder="输入报价" size="small" style="width: 86px;"></el-input>
      </div>
    ...
    <div class="result-list">
      <div class="btn-group">
        ...
        <el-button type="primary" size="small" @click="exportStar">导出</el-button> <!--  :disabled="selectedStar.length === 0" -->
      ...
      <div class="result-table">
        ...
        >
          ...
          <el-table-column
            ...
            width="120">
            <template slot-scope="scope">
              <el-rate disabled text-color="#ff9900" void-color="#E7E7E7" v-model="scope.row.responsivity"></el-rate>
            </template>
          </el-table-column>
          <el-table-column
            prop="fans_number"
    ...
    <el-dialog
      ...
      <div class="cutom-header" slot="title">
        <span :class="['dialog-tab', dialogShowBlockIndex === 0 ? 'active' : '']" @click="dialogShowBlockIndex = 0">新增主播</span>
        <span :class="['dialog-tab', dialogShowBlockIndex === 1 ? 'active' : '']" @click="dialogShowBlockIndex = 1">批量导入</span>
      </div>
      <div v-show="dialogShowBlockIndex === 0">
        <el-form :model="starInfoForm" :inline="true" size="small" label-position="top" ref="starInfoForm">
          <el-row>
            <el-form-item class="label-item" label="主播ID" prop="star_id" :rules="[{required: true, message: '淘客ID不能为空'}]">
      ...
      <div class="upload-wrap" v-show="dialogShowBlockIndex === 1">
        <el-row>
          <el-upload :action='""' :http-request="uploadStarFile" :show-file-list="false" class="upload-btn">
            <el-button size="mini" type="primary" :loading="uploadLoading" style="border-right: none;">
              <div class="upload-icon"></div>
              <p class="icon-title">批量导入主播基本信息</p>
              <p class="icon-tip">点击此处选择要导入的文件</p>
            </el-button><!-- 批量导入主播基本信息 -->
          </el-upload>
          <el-upload :action='""' :http-request="uploadStarCostFile" :show-file-list="false" class="upload-btn">
            <el-button size="mini" type="primary" :loading="uploadCostLoading">
              <div class="upload-icon"></div>
              <p class="icon-title">批量导入主播成本报价</p>
              <p class="icon-tip">点击此处选择要导入的文件</p>
            </el-button><!-- 批量导入主播成本报价 -->
          </el-upload>
        </el-row>
        <el-row class="download-btns">
          <el-button size="mini" type="info" plain @click="downloadModelxlsx">下载主播基本信息模板</el-button>
          <el-button size="mini" type="info" plain @click="downloadStarCostModel">下载主播成本报价模板</el-button>
        </el-row>
      </div>
    </el-dialog>
    <!-- 批量导入弹窗 -->
...
<script>
import { fansNumberOptions, mixSpeicalOptions, categoryOptions, salesPriceOptions } from '@/const/options'
...
export default {
  ...
  methods: {
    ...
    queryStar() {
      ...
      // 处理两个价格
      querystarpass[this.condition.cost_mix_or_special + '_min_cost'] = this.condition.min_cost || null
      querystarpass[this.condition.cost_mix_or_special + '_max_cost'] = this.condition.max_cost || null
      querystarpass[this.condition.price_mix_or_special + '_min_price'] = this.condition.min_price || null
      querystarpass[this.condition.price_mix_or_special + '_max_price'] = this.condition.max_price || null
      // debugger
    ...
    exportStar() {
      if (this.selectedStar.length === 0) {
        ...
        // })
        let ids = this.selectedStar.map(item => item.star_id)
        // debugger
        let starinfopass = {
      ...
      return (
        <span>品类
          <el-tooltip placement="top" effect="light">
            <div slot="content">
              <p>系统自动获取，该主播在淘宝直播间，历史出售的商品品类中出现频次最多的前三个品类。</p>
              <p>例：该场直播间共挂出10个商品链接，其中5个美妆商品，2个女装商品，2个个护商品，1个零食商品，那么该主播品类为：美妆/女装/个护。</p>
            </div>
