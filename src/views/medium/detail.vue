  ...
  <div class="medium-detail-wrapper" v-loading="detailLoading">
    <div class="common-block-wrapper">
      <div class="detail-title">
        ...
        <span v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.update_company)" class="icon-item" @click="editCompany">
          <el-tooltip content="编辑公司详情" placement="top" effect="light">
            <i class="gm-icon gm-icon-edit"></i>
          </el-tooltip>
        </span>
        <span v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.del_company)" class="icon-item" @click="delCompany">
          <el-tooltip content="删除公司" placement="top" effect="light">
            <i class="gm-icon gm-icon-delete"></i>
          </el-tooltip>
        </span>
      </div>
      <div class="detail-content">
        <el-row class="content-box">
          <el-col :span="9" class="company-box">
            <div class="logo-box">
              <img class="logo" v-if="companyForm.logo" :src="fixFileToken(companyForm.logo)" alt="">
            ...
            <div class="text-box">
              ...
              <p class="contact-text">
                ...
                <span class="value">
                  {{companyForm.contact_person || defaultVal}} <template v-if="companyForm.contact_no">（{{companyForm.contact_no}} ）</template>
                </span>
              ...
              <p class="contact-text">
                ...
                <span class="value" v-if="companyForm.contact_email">{{companyForm.contact_email}}</span>
                <span class="value" v-else>{{defaultVal}}</span>
              </p>
              <p class="contact-text">
        ...
        <div class="file-box">
          ...
          <span class="file-name" v-if="quotationName" @click="previewFile">{{quotationName}}</span>
          ...
          <div v-if="quotationName">
            <a @click.stop :href="fixFileToken(companyForm.quotation)" download target="_blank"><el-button type="primary" size="small">下载报价单</el-button></a>
          </div>
        </div>
      </div>
    </div>
    <div class="common-block-wrapper introduction-wrapper">
      ...
      <div class="introduction-content" v-if="companyForm.description || (companyForm.description_content && companyForm.description_content.length > 0)">
        <div class="introduction-text" v-html="companyForm.description.replace(/\n/g,'<br>')">
        </div>
        <div class="introduction-img" style="padding-top: 10px;" v-if="companyForm.description_content && companyForm.description_content.length > 0">
          <div style="padding-bottom: 20px; min-height: 50px;" v-for="item in companyForm.description_content" :key="item"><img v-lazy="fixFileToken(item)" alt=""></div>
        </div>
      </div>
      <div class="content-loading" v-else-if="companyForm.description_file && companyForm.description_content.length === 0">
        数据上传中，请稍后刷新查看
      </div>
    ...
    <add-company v-if="companyVisible" @close="close" @save="saveCompany" :companyForm="companyFormEdit" :id="id"></add-company>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    platforms() {
      ...
      if (this.companyForm.platforms) {
        ret = this.companyForm.platforms.split(',').map(item => {
          let arr = this.cooperationPlatformList.filter(obj => {
            return obj.type === parseInt(item)
          })
          return arr[0].value
        })
      }
    ...
    addrDetail() {
      return this.companyForm.province + this.companyForm.city + this.companyForm.county + this.companyForm.address
    },
    quotationName() {
  ...
  methods: {
    ...
    downloadQuotation() {
      if (this.companyForm.quotation) {
        window.open(this.fixFileToken(this.companyForm.quotation))
      }
    },
    fixFileToken
  },
