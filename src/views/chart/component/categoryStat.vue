<template>
  <section class="cate-stat">
    <div>
      <div style="margin:10px;overflow:hidden">
        ...
        <div class="category-dropdown" style="float:right">
          <el-dropdown trigger="click" :hide-on-click="false" @visible-change="isRightArrow = !isRightArrow">
            <span class="el-dropdown-link">选择维度
              <i :class="isRightArrow ? 'el-icon-arrow-right':'el-icon-arrow-down'"></i>
            </span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(item,index) in categoryStatDimension" :style="{color:item.color}"
                :key="index" @click.native="selectDimen(item, index)">
        ...
        <el-dropdown class="cate-recent-day" trigger="click" :hide-on-click="true">
          <span class="el-dropdown-link">{{categorystatRecentDay}}<i class="el-icon-arrow-down"></i></span>
          <el-dropdown-menu slot="dropdown" style="float:left;margin-left:12px">
            <el-dropdown-item v-for="(item,index) in categoryRecentDayDimension" :key="index" @click.native="selectDay(item, index)">
              {{item.value}}
            </el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
    ...
    <!-- <div class="cate-recent-day">
      <el-dropdown trigger="click" :hide-on-click="true">
        <span class="el-dropdown-link">{{categorystatRecentDay}}<i class="date-form-icon"></i></span>
        <el-dropdown-menu slot="dropdown" style="float:left;margin-left:12px">
          <el-dropdown-item v-for="(item,index) in categoryRecentDayDimension" :key="index" @click.native="selectDay(item, index)">
            {{item.value}}
          </el-dropdown-item>
...
export default {
  ...
  methods: {
    selectDimen(item, index) {
      let currentColor = this.categoryCurrentColor[index]
      if (item.color !== '#999') {
        item.color = '#999'
    ...
    initCateChart() {
      ...
      if (this.categoryStatChartInfo.length === 0) {
        ...
        let categoryStatChartShowInfo = this.categoryStatChartInfo[this.currentDay]
        for (var i = 0, len = categoryStatChartShowInfo.categorys_average_watch_persons.length; i < len; i++) {
          categoryStatChartShowInfo.categorys_average_watch_persons[i] = parseInt(categoryStatChartShowInfo.categorys_average_watch_persons[i])
        }
        this.cateChart.setOption({
          color: ['#4dcdb1', '#7484b7'],
          legend: {
            show: true, x: 'center', top: '-999'
          },
          tooltip: {
            trigger: 'axis',
            ...
            axisPointer: {
              type: 'shadow', shadowStyle: {color: 'rgba(69, 93, 161, 0.1)'}
            },
            formatter (params) {
          ...
          grid: {
            x: 72, x2: 46, y: 36, y2: 46, borderWidth: 0
          },
          // x轴设置
          xAxis: {
            type: 'category',
