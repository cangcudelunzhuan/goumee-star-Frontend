<template>
  <section class="star-personal-section page-section">
    <el-row class="stat-top">
      <section class="daily-stat" v-loading="dailyLoading">
        <div>
          <div class="daily-stat-top">
            <span>每日统计</span>
            <el-dropdown trigger="click" :hide-on-click="true" class="recent-day-parent" placement="bottom-start">
              <span class="el-dropdown-link recent-day">{{recentDay}}<i class="el-icon-arrow-down"></i></span>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="item in recentDayDimension" :key="item.value" @click.native="selectedDateDimension(item)">{{item.value}}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
            <div>
              <el-dropdown trigger="click" :hide-on-click="false" @visible-change="isSelect = !isSelect">
                <span class="el-dropdown-link" style="cursor:pointer;font-size:12px;color:#666">
                  选择维度
                  <i class="el-icon--right" :class="isSelect ? 'el-icon-arrow-right' : 'el-icon-arrow-down'"></i>
                </span>
                <el-dropdown-menu slot="dropdown" class="daily-dimen">
                  <el-dropdown-item v-for="(item,index) in periodStatDimension" :style="{color:item.color}" :key="index" :class="dimensionCls ? 'is-selected' : 'no-selected'" @click.native="selectedDimension(item,index)">
          ...
          <!-- <div style="margin-left:10px">
            <el-dropdown trigger="click" :hide-on-click="true" class="recent-day-parent" placement="bottom-start">
              <span class="el-dropdown-link recent-day">{{recentDay}}<i class="date-form-icon"></i></span>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="item in recentDayDimension" :key="item.value" @click.native="selectedDateDimension(item)">{{item.value}}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
      ...
      <cateStatChart ref="cateStateChart" v-if="$props.starName !== ''" :categoryStatChartInfo="categoryStatChartInfo" v-loading="categoryLoading"></cateStatChart>
    </el-row>
    <el-row>
      <section class="display-stat" v-loading="displayLoading">
        <div style="overflow:hidden">
          ...
          <div class="dimension-dropdown">
            <el-dropdown trigger="click" :hide-on-click="false" @visible-change="isOpenDisplayDimen = !isOpenDisplayDimen">
              <span class="el-dropdown-link">
                选择维度
                <i class="el-icon--right" :class="isOpenDisplayDimen ? 'el-icon-arrow-down' : 'el-icon-arrow-right'"></i>
              </span>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="(item, index) in displayStatDimension" :style="{color:item.color}" :key="index" @click.native="selectedDisplayDimesion(item,index)">
                  <i class="circleDot" :style="{ background: item.color}"></i>
                  {{item.value}}
                </el-dropdown-item>
              </el-dropdown-menu>
          ...
          <el-dropdown class="display-stat-day" trigger="click" :hide-on-click="true" placement="bottom-start">
            <span class="el-dropdown-link">{{displayRecentDay}}<i class="el-icon-arrow-down"></i></span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(item, index) in displayRecentDayDimension" :key="index" @click.native="selectedDisplayDay(item,index)">
                {{item.value}}
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        ...
        <!-- <div>
          <el-dropdown class="display-stat-day" trigger="click" :hide-on-click="true" placement="bottom-start">
            <span class="el-dropdown-link">{{displayRecentDay}}<i class="display-icon"></i></span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(item, index) in displayRecentDayDimension" :key="index" @click.native="selectedDisplayDay(item,index)">
                {{item.value}}
              </el-dropdown-item>
    ...
    <el-row>
      <section class="categories-ranking" v-loading="smallCateLoading">
        <div class="categories-ranking-top">
          ...
          <el-dropdown class="cate-ranking-recent" trigger="click" :hide-on-click="true" placement="bottom-start">
            <span class="el-dropdown-link">{{smallCateDimenRecent}}<i class="el-icon-arrow-down"></i></span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(item,index) in smallCategoryDaydimension" :key="index" @click.native="selectedSmCateDay(item, index)">{{item}}</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>
...
<script>
import {queryStarStat, periodStatistics, displayStatistics, queryCategory, smallCategory} from '@/api/stat'
...
export default {
  components: { cateStatChart, fanPortrait },
  props: ['starName'],
  data () {
    return {
      domain,
  ...
  methods: {
    mountedSearch () {
      ...
      //   this.$refs.cateStateChart.winResize()
      // })
    },
    // resize事件
    winResize () {
    ...
    selectedDisplayDimesion (item, index) {
      let currentColor = this.displayCurrentColor[index]
      if (item.color !== '#999') {
        item.color = '#999'
              ...
              if (res.length === 0) {
                ...
                res.reverse().forEach((item) => {
                  if (item.watch_person_num_by_hour) {
                    ...
                    this.periodStatChartInfo.averageWatchPersonByHour.push(item.watch_person_num_by_hour)
                    this.periodStatChartInfo.averageWatchNumberByHour.push(item.watch_num_by_hour)
                    this.periodStatChartInfo.averageWatchPersonByDisplay.push(item.watch_person_num_by_display)
                    this.periodStatChartInfo.averageWatchNumberByDisplay.push(item.watch_num_by_display)
    ...
    selectedDimension (item, index) {
      ...
      let currentColor = this.periodCurrentColor[index]
      if (item.color !== '#999') {
        item.color = '#999'
      } else if (item.color === '#999') {
        item.color = currentColor
    ...
    searchStar () {
      if (this.searchStarInfo !== '' && this.searchStarInfo !== null) {
        ...
        queryStarStat(searchStarPass).then(response => {
          let data = response.data
          // // 判断是否为合作主播
          ...
          if (data.success) {
            ...
            queryCategory(queryCategoryPass).then(response => {
              ...
              if (data.success) {
                if (data.data.data.length === 0) {
                  ...
                  this.categoryStatChartInfo = [res.category_info_15, res.category_info_30, res.category_info_90]
                  this.categoryLoading = false
                }
              }
            })
            ...
            this.displayLoading = true
            displayStatistics(displayStatpass).then(response => { // 场次统计
              let data = response.data
              if (data.success) {
                if (data.data.data.length === 0) {
                  this.displayStatChart = echarts.init(document.getElementById('displayStatChart'))
            ...
            smallCategory(starnamepass).then(response => {
              ...
              if (result.success) {
                if (result.data.data.length === 0) { // 有主播信息-无数据
    ...
    initDisplayStatChart () {
      if (this.isDestroyed) return
      let showLabel = this.displayStatChartInfo.label.map(item => {
        if (item === '') item = '-'
        return item
      })
      this.displayStatChart = echarts.init(document.getElementById('displayStatChart'))
    ...
    initChildCateStatChart () {
      ...
      this.childCateStatChart = echarts.init(document.getElementById('childCateStatChart'))
      this.childCateStatChart.clear()
      if (this.smallCategoryStatChartInfo[this.smallCateSelectedDays].small_categorys.length === 0) {
        this.smallCategoryNoDataShow()
        this.smallCateLoading = false
      } else {
        let smallCateShowInfo = this.smallCategoryStatChartInfo[this.smallCateSelectedDays]
        for (var i = 0, len = smallCateShowInfo.average_watch_persons.length; i < len; i++) {
          smallCateShowInfo.average_watch_persons[i] = parseInt(smallCateShowInfo.average_watch_persons[i])
        }
        let options = {
