  ...
  <section>
      ...
      <el-form :inline="true" :model="searchDisplayForm" class="search-display-item" label-position="right" label-width="84px">
        ...
        <el-row type="flex" justify="space-between">
          ...
          <el-form-item label=" ">
            ...
            <el-button style="width:80px" size="small" @click="resetCondition">重置</el-button>
          </el-form-item>
        </el-row>
      </el-form>
    </el-row>
    ...
    <el-row style="background:#fff;padding:12px" class="live-records-table">
      <el-row v-if="canExportDisplay" class="search-display-button">
        <el-button type="primary" style="width:80px" size="small" @click="exportStarSubmit">导出</el-button>
      </el-row>
      <el-table
        ...
        <el-table-column type="selection" v-if="canExportDisplay">
        </el-table-column>
        ...
        <el-table-column label="直播类型" min-width="100">
          <template slot-scope="scope">
            <p>{{ getDisplayType(scope.row.displayType) }}
        ...
        <el-table-column prop="pv" label="总PV/万" min-width="76" align="right">
        </el-table-column>
        ...
        <el-table-column label="录入人" min-width="124" show-overflow-tooltip align="right">
          <template slot-scope="scope">
            <div>
              ...
              <p style="margin:0;font-size:12px;color:#999;">{{ addDateFormat(scope.row.addTime) }}</p>
            </div>
          </template>
        </el-table-column>
...
import {displayTypeOptions, isPresellOptions, isDisplayOptions, latestDisplayTimeOptions} from '@/const/options' // , presellSelectOption
...
export default {
  ...
  methods: {
    ...
    exportStarSubmit: function () {
      ...
      if (this.isSameStar) {
        ...
        exportDisplays(exportpass).then((response) => {
          ...
          if (data.success) {
            let url = this.domain + '/api/star/export_star_file?timestamp=' + data.data
            window.open(url)
    ...
    handleDisplayClick (row) {
      ...
      link.setAttribute('href', `/product-detail?displayId=${row.id}&currentstarId=${row.starId}&currentStarName=${row.starName}`)
