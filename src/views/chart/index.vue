<template>
  <section class="star-personal-section page-section">
    <!-- <el-row class="top-el-row">
      <div>
        <input type="text" placeholder="请输入主播昵称" @keyup.enter="searchStar" v-model="searchStarInfo" class="search-input" :class="isOnfoucs ? 'onfoucs-style' : 'onblur-style'" @focus="isOnfoucs = true" @blur="isOnfoucs = false">
        <button class="search-button" @click="searchStar"></button>
      </div>
    </el-row> -->
    <!-- <el-row class="show-info-stat" :class="isConStar ? 'is-con-style' : 'not-con-style'">
      <el-col class="info-left">
        <div class="info-left-child">
          <div class="head-img" v-show="isConStar"><img :src="showStarInfo.imgSrc" alt=""></div>
          <p class="info-desc" :class="isConStar ? 'info-desc-con': ''">{{showStarInfo.showStarName}}</p>
          <p :class="isConStar ? 'info-attr-con':''">
            <span class="info-attr">主播ID：{{showStarInfo.showStarId}}</span>
          </p>
          <p v-show="isConStar" :class="isConStar ? 'info-attr-con':''">
            <span class="info-attr">
              淘客ID：{{showStarInfo.wangwang === '' ? '--' : showStarInfo.wangwang}}
            </span>
          </p>
          <p v-show="isConStar" :class="isConStar ? 'info-attr-con-rate':''">
            配合度
            <el-rate disabled style="display:inline-block;vertical-align: middle;" v-model="showStarInfo.Cooperation"></el-rate>
          </p>
          <p v-show="isConStar" class="info-phone-wechat">
            <i class="icon-phone"></i>
            <span>{{showStarInfo.phoneNum === '' ? '-':showStarInfo.phoneNum}}</span>
          </p>
          <p v-show="isConStar" class="info-phone-wechat">
            <i class="icon-wechat"></i>
            <span>{{showStarInfo.weChat=== '' ? '-':showStarInfo.weChat}}</span>
          </p>
        </div>
      </el-col>
      <el-col class="info-line" v-show="isConStar"></el-col>
      <el-col class="info-right">
        <div class="info-desc-par">
          <p class="info-desc info-desc-num fans-color">{{showStarInfo.showStarFuns}}</p>
          <span class="info-attr">粉丝数/万</span>
        </div>
        <div class="info-desc-par">
          <p class="info-desc info-desc-num display-color">
            {{showStarInfo.showStarDisplay}}
            <i class="el-icon-arrow-right" v-show="isConStar" @click="goDisplayDetail(showStarInfo.showStarName)"></i>
          </p>
          <span class="info-attr">直播场次数/场</span>
        </div>
        <div class="info-desc-par">
          <p class="info-desc info-desc-num score-color">{{showStarInfo.showStarScore}}</p>
          <span class="info-attr">流量评分</span>
        </div>
        <div class="info-desc-par" style="border:0">
          <p class="info-desc org-color">{{showStarInfo.showStarOrganize}}</p>
          <span class="info-attr">当前所属机构</span>
        </div>
        <div v-show="isConStar" style="height: 12px;background:#f1f2f3"></div>
        <div v-show="isConStar" style="height:200px;">
          <el-row type="flex" justify="space-between" class="basic-right-bot">
            <div class="pro-attr">
              <div class="pro-attr-top">
                <div class="pro-attr-cost-price">
                  <p class="cost-price-tit">主播成本及报价/元</p>
                  <div class="cost-price-desc">
                    <div class="cost-price-part">
                      <p>混播成本&#12288;<span>{{showStarInfo.starMixCost === 0 ? ' -' : showStarInfo.starMixCost}}</span></p>
                      <p>混播报价&#12288;<span>{{showStarInfo.starMixPrice === 0 ? ' -':showStarInfo.starMixPrice}}</span></p>
                    </div>
                    <div class="cost-price-part">
                      <p>专场成本&#12288;<span>{{showStarInfo.starSpecialCost === 0 ? ' -':showStarInfo.starSpecialCost}}</span></p>
                      <p>专场报价&#12288;<span>{{showStarInfo.starSpecialPrice === 0 ? ' -':showStarInfo.starSpecialPrice}}</span></p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="pro-attr-recent-pv">
                <div style="width:49%;float:left">
                  <p>商品属性</p>
                  <span>品&#12288;类 &#12288;<strong> {{ showStarInfo.category === '' ? '-':showStarInfo.category }} </strong></span>
                  <span>客单价 &#12288; <strong> {{ getSales(showStarInfo.unitPrice) }} </strong></span>
                </div>
                <div style="width:49%;float:left">
                  <p>最近15场场均PV/万</p>
                  <span class="mix-spec-pv" v-show="showStarInfo.mixed.length === 0">混&#12288;播 &#12288; <strong>0</strong></span>
                  <span class="mix-spec-pv" v-show="showStarInfo.mixed.length === 0">专&#12288;场 &#12288; <strong>0</strong></span>
                  <span class="mix-spec-pv" v-if="showStarInfo.mixed.length > 0">{{showStarInfo.mixed[0][0]}} &#12288;<strong>{{showStarInfo.mixed[0][1]}}</strong></span>
                  <span class="mix-spec-pv" v-if="showStarInfo.mixed.length > 1">{{showStarInfo.mixed[1][0]}} &#12288;<strong>{{showStarInfo.mixed[1][1]}}</strong></span>
                </div>
              </div>
            </div>
            <div style="width:12px;background:#f1f2f3;height:200px"></div>
            <div v-loading="pieLoading" class="pie-chart">
              <div v-show="isConStar" id="pieStatChartsPar">
                <div id="pieStatCharts"></div>
              </div>
              <div class="click-num-noData" v-show="showStarInfo.latestClickNumPieData.length === 0">
                最近30款产品点击数/次
                <span>还没有开播 ~</span>
              </div>
            </div>
    ...
    <div class="new-star-info-header">
      <div class="header-top">
        ...
        <span>主播详情</span>
        <p v-show="isConStar" class="add-modify-by"><em>录入人：{{showStarInfo.addBy || '--'}}</em>|<em>最终修改人：{{showStarInfo.modifyBy || '--'}}</em></p>
        <!-- <div class="btns">
        ...
        <div class="opt-btns" v-show="isConStar">
          <span v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.update_kol)"><i class="gm-icon gm-icon-edit" @click="openSaveDialog"></i></span>
          <span v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.del_kol)"><i class="gm-icon gm-icon-delete" @click="deleteOneStar"></i></span>
        </div>
      </div>
      <el-row class="star-info">
        <el-col :span="8" class="border-right-gray">
          ...
          <div class="star-info-detail">
            ...
            <div class="info-box" :style="`height: ${shrinkState ? '40' : '100'}px;`">
              ...
              <p class="info-item">配合度：<el-rate disabled style="display:inline-block;vertical-align: middle;" v-model="showStarInfo.Cooperation"></el-rate></p>
              <p class="info-item">淘客ID：{{showStarInfo.wangwang === '' ? '--' : showStarInfo.wangwang}}</p>
              <p class="info-item">手机号：{{showStarInfo.phoneNum === '' ? '--':showStarInfo.phoneNum}}</p>
              <p class="info-item">微信号：{{showStarInfo.weChat=== '' ? '--':showStarInfo.weChat}}</p>
            </div>
            <span class="shrink-btn" @click="handleShrink">{{shrinkState ? '更多资料' : '收起'}} <i v-show="shrinkState" class="el-icon-arrow-down"></i><i v-show="!shrinkState" class="el-icon-arrow-up"></i></span>
          </div>
        </el-col>
        <el-col :span="4" class="data-box-wrap">
          <div class="data-box">
            <p class="data-val">{{showStarInfo.showStarFuns === '--' ? showStarInfo.showStarFuns : formatNumber((showStarInfo.showStarFuns * 10000).toFixed(0))}}</p>
        ...
        <el-col :span="4" class="data-box-wrap">
          <div class="data-box">
            <p class="data-val" :style="showStarInfo.showStarOrganize === '未知' ? 'color: #CBCBCB;' : ''">{{showStarInfo.showStarOrganize}}</p>
            ...
            <el-row>
              <el-col class="info-block-box-wrap" :span="8">
                <div class="info-block-box">
                  ...
                  <p class="info-block-data">
                    ...
                    <span class="info-block-data-val">{{showStarInfo.starMixCost === 0 ? '--' : showStarInfo.starMixCost}}</span>
                  </p>
                  <p class="info-block-data">
                    <span class="info-block-data-label">混播报价</span>
                    <span class="info-block-data-val">{{showStarInfo.starMixPrice === 0 ? '--':showStarInfo.starMixPrice}}</span>
                  </p>
                  <p class="info-block-data">
                    <span class="info-block-data-label">专场成本</span>
                    <span class="info-block-data-val">{{showStarInfo.starSpecialCost === 0 ? '--':showStarInfo.starSpecialCost}}</span>
                  </p>
                  <p class="info-block-data">
                    <span class="info-block-data-label">专场报价</span>
                    <span class="info-block-data-val">{{showStarInfo.starSpecialPrice === 0 ? '--':showStarInfo.starSpecialPrice}}</span>
                  </p>
                </div>
              </el-col>
              <el-col class="info-block-box-wrap" :span="8">
                <div class="info-block-box">
                  ...
                  <p class="info-block-data">
                    ...
                    <span class="info-block-data-val">{{showStarInfo.mixed === '--' ? '--' : (showStarInfo.mixed[1] ? showStarInfo.mixed[1][1] : '0')}}</span>
                  </p>
                  <p class="info-block-data">
                    <span class="info-block-data-label">混播</span>
                    <span class="info-block-data-val">{{showStarInfo.mixed === '--' ? '--' : (showStarInfo.mixed[0] ? showStarInfo.mixed[0][1] : '0')}}</span>
                  </p>
                </div>
              </el-col>
              <el-col class="info-block-box-wrap no-border" :span="8">
                <div class="info-block-box" style="width: 90%;">
                  ...
                  <p class="info-block-data">
                    <span class="info-block-data-label" style="width: 42px; display: inline-block;">品类</span>
                    <span class="info-block-data-val">
                      <p v-if="!showStarInfo.star_category" class="star-name" style="display: inline; position: static;">--</p>
                      <el-tooltip
    ...
    <el-dialog title="" :visible.sync="saveStarDialogVisible" class="edit-star-dialog" :close-on-click-modal="false" :close-on-press-escape="false" @closed="editDialogClose('starInfoForm')"
      append-to-body custom-class="dialog-min-width">
      <div class="cutom-header" slot="title"><span>编辑主播信息</span></div>
      <el-form :model="starInfoForm" :inline="true" size="small" label-position="top" ref="starInfoForm">
        <el-row>
          <el-form-item label="主播ID" prop="star_id" :rules="[{required: true, message: '淘客ID不能为空'}]">
            <el-input name="star_id" v-model="starInfoForm.star_id" type="number"></el-input>
          </el-form-item>
          <el-form-item label="手机号" prop="star_mobile" :rules="[{required: true, message: '手机号不能为空'}]">
            <el-input name="star_mobile" v-model="starInfoForm.star_mobile" type="number"></el-input>
          </el-form-item>
        </el-row>
        <el-row>
          <el-form-item label="配合度" prop="responsivity" :rules="[{required: true, message: '配合度不能为空'}]">
            <el-rate v-model="starInfoForm.responsivity" show-text name="responsivity"></el-rate>
          </el-form-item>
          <el-form-item label="粉丝数" prop="fans_number" :rules="[{required: true, message: '粉丝量不能为空'}]">
            <el-input name="fans_number" v-model="starInfoForm.fans_number" type="number">
              <template slot="append">万</template>
            </el-input>
          </el-form-item>
        </el-row>
        <el-row>
          <el-form-item label="昵 称" prop="star_name" :rules="[{required: true, message: '昵称不能为空'}]">
            <el-input name="star_name" v-model="starInfoForm.star_name"></el-input>
          </el-form-item>
          <el-form-item label="微信号" prop="star_wechat" :rules="[{required: true, message: '微信号不能为空'}]">
        ...
        <el-row>
          ...
          <el-form-item label="客单价" prop="sales_price_period" :rules="[{required: true, message: '客单价不能为空'}]">
            <el-select v-model="starInfoForm.sales_price_period" placeholder="请选择" name="sales_price_period">
              <el-option v-for="item in salesPriceOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="editDialogClose('starInfoForm');saveStarDialogVisible=false">取消</el-button>
        <el-button type="primary" @click="submitSaveStar(starInfoForm, 'starInfoForm')" size="small">保存</el-button>
      </div>
    </el-dialog>
  </section>
</template>
...
import cateStatChart from './component/categoryStat'
...
export default {
  components: { cateStatChart, fanPortrait, liveRecords, dataStatistics },
  data () {
    return {
      RIGHT_CODE,
      domain,
  ...
  methods: {
    ...
    goDisplayDetail (value) {
      ...
      this.$router.push({path: '/operation-center/display-management', query: {'starName': value}})
    },
    getSales (value) {
    ...
    async searchStar () {
      if (this.searchStarInfo !== '' && this.searchStarInfo !== null) {
        ...
        try {
          let response = await queryStarStat(searchStarPass)
          let data = response.data
          if (data.success) {
        ...
        // 是合作主播时查询主播详细信息
        queryStars(searchConditions).then((response) => {
          ...
          if (res) {
            ...
            if (!this.showStarInfo.imgSrc) this.showStarInfo.imgSrc = this.domain + '/template/default_avatar.jpeg'
          }
    ...
    initPieStatChart () {
      ...
      this.pieStatChart.setOption({
        color: ['#9dd7ff', '#fda9c0', '#8ee7d4', '#ffbbf3', '#fbe3b3', '#93a4dc', '#ffb69c', '#dfc3ff', '#a8c5ff', '#b2dbad', '#f7aeb3'],
        title: {
          text: '最近30款产品点击数/次',
          textStyle: {
            color: '#666',
    ...
    openSaveDialog() {
      ...
      let getStarCostArr = ['star_mix_cost', 'star_mix_price', 'star_special_cost', 'star_special_price']
