<template>
  <div>
    <!-- 表单验证 ref prop rules -->
    <el-form size="small" :inline="true" :model="displayInfoForm" label-width="80px" label-position="top" ref="displayInfoForm">
      <el-form-item label="直播标题">
        <el-input name="title" v-model="displayInfoForm.title"></el-input>
      </el-form-item>
      <el-form-item label="主播昵称">
        <el-input name="starName" v-model="displayInfoForm.star_name" disabled placeholder="请输入内容"></el-input>
      </el-form-item>
      <el-form-item label="主 播 ID">
        <el-input name="starId" disabled v-model="displayInfoForm.star_id" placeholder="请输入内容" type="number"></el-input>
      </el-form-item>
      ...
      <el-form-item label="场次UV">
        <el-input type="number" placeholder="请输入内容" v-model="displayInfoForm.uv">
          <template slot="append">次</template>
        </el-input>
      </el-form-item>
      <el-form-item label="直播类型" prop="display_type" :rules="[{required: true, message: '直播类型是必填项'}]">
        <el-select name="displayType" v-model="displayInfoForm.display_type" placeholder="请选择直播类型">
      ...
      <el-form-item label="是否预售场" prop="is_presell" :rules="[{required: true, message: '是否预售场为必填项'}]">
        <el-select placeholder="请选择是否为预售场" name="is_presell" v-model="displayInfoForm.is_presell">
...
export default {
  ...
  mounted () {
    this.$emit('displayFormRef', this.$refs['displayInfoForm'])
  },
  methods: {
