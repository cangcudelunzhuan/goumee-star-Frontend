<template>
  <section>
    <el-row :class="['search-display-row', {'jianxiao': ctype == 'cooperative'}]">
      <el-form :inline="true" :model="searchDisplayForm" class="search-display-item" label-position="right" label-width="84px">
        <el-row type="flex" justify="space-between">
          <el-form-item label="主播昵称">
            <el-select v-model="searchDisplayForm.starName" size="small" filterable clearable>
              <el-option v-for="(item, index) in allStars" :key="index" :value="item.value" :label="item.value"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="最近直播">
            <el-select placeholder="选择最近直播日期" v-model="searchDisplayForm.latestDisplayTime" size="small" clearable>
        ...
        <el-row type="flex" justify="space-between">
          ...
          <el-form-item label="是否预售">
            <el-select placeholder="选择是否预售" v-model="searchDisplayForm.isPresell" size="small" clearable>
          ...
          <el-form-item label="是否出单">
            <el-select placeholder="选择是否已出单" size="small" clearable v-model="searchDisplayForm.isDisplay">
      ...
      <el-row class="search-display-button">
        ...
        <el-button style="width:80px" type="primary" size="small" @click="searchDisplay">查询</el-button>
        <el-button style="width:80px" size="small" @click="searchReset">重置</el-button>
      </el-row>
    </el-row>
    <el-dialog custom-class="dialog-min-width-tip" :visible.sync="lotImportDialogVisible" title="批量导入" width="22%" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-row style="text-align:center">
        <el-upload v-if="isAdminUser" class="display-import-button" ref="upload" :action='""' :http-request="uploadDisplay" :show-file-list="false" multiple :file-list="fileList">
          <el-button size="small" type="primary" :loading="uploadLoading" style="width:auto">导入场次</el-button>
        </el-upload>
      </el-row>
      ...
      <el-row style="text-align:center;margin-top:12px">
        <el-button size="small" type="primary" @click="downloadModelDesc">下载使用说明文档</el-button>
      </el-row>
    </el-dialog>
    ...
    <el-dialog
      ...
      <div class="addDisplay-header" slot="title">编辑场次</div>
      <el-form :inline="true" size="small" :model="displayInfoForm" label-width="80px" label-position="top" ref="displayInfoForm">
        ...
        <el-form-item label="主播昵称" prop="starName" :rules="[{required: true, message: '主播昵称是必填项'}]">
          ...
          :trigger-on-focus="false" @select="handleSelectAdd" :fetch-suggestions="querySearchAdd" @keyup.native="starKeyUp"
    ...
    <!-- main-table -->
    <el-row style="background:#fff;padding:13px" :class="{'jianxiao': ctype == 'cooperative'}">
      <el-row class="search-display-button" style="margin-bottom: 12px;">
        <el-button
          ...
          type="primary"
          size="small"
          @click="addDisplay(); addAndImmportDialogVisiable = true">新增场次</el-button>
        <div v-if="ctype == 'cooperative'" style="width: 100%; height: 1px; background: rgba(232,232,232,1); margin-bottom: 15px; margin-top: -20px;"></div>
        <el-button
          v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.export_display)"
          type="primary"
          style="width:80px"
          size="small"
          @click="exportStarSubmit">导出</el-button>
        <el-button
      ...
      <el-table
        ...
        <el-table-column prop="title" min-width="160" label="直播标题/日期/类型">
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.title" placement="top" effect="light">
              <span class="display-title">{{scope.row.title === '' ? '-' : scope.row.title}}</span>
            </el-tooltip>
            <p style="font-size:12px;color:#999;">{{dateFormat(scope.row, {property: 'displayTime'})}}</p>
            <p>
              <span class="pre-tag">{{ getDisplayType(scope.row.displayType) }}</span>
        ...
        <el-table-column label="主播昵称/ID" min-width="110">
          <template slot-scope="scope">
            <div>
              <el-tooltip :content="scope.row.starName + ' ' + scope.row.starId" placement="top" effect="light">
                <p class="text-overflow-cls">{{scope.row.starName}}</p>
              </el-tooltip>
              <p class="text-overflow-cls">{{scope.row.starId}}</p>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="出单状态" width="100" >
          <template slot-scope="scope">
            ...
            <el-tooltip
              v-if="(scope.row.displaySalesAmount > 0 || scope.row.displaySalesNum > 0) && scope.row.isDisplay === 1"
              placement="right"
        ...
        <el-table-column label="总销售情况" width="120" :render-header="TotalSales">
          <template slot-scope="scope">
            <div v-if="(scope.row.displaySalesAmount !== 0 || scope.row.isDisplay === 2) && (scope.row.displaySalesNum !== 0 || scope.row.isDisplay === 2)">
              <p class="sales-desc" v-show="scope.row.displaySalesAmount !== 0 || scope.row.isDisplay === 2">销售额: {{scope.row.displaySalesAmount === -1 ? '未出单':scope.row.displaySalesAmount + ' 元'}}</p>
              <p class="sales-desc" v-show="scope.row.displaySalesNum !== 0 || scope.row.isDisplay === 2">销售量: {{scope.row.displaySalesNum === -1 ? '未出单': scope.row.displaySalesNum + ' 件'}}</p>
            </div>
            <span v-else>--</span>
          </template>
        </el-table-column>
        <el-table-column prop="products_num" label="总商品数" width="100" align="center">
        </el-table-column>
        <el-table-column prop="pv" label="总PV/万" width="100" align="center">
        </el-table-column>
        <el-table-column prop="products_click_num" label="总点击数" width="100" align="center">
        </el-table-column>
        <el-table-column label="录入人/日期" width="110" show-overflow-tooltip align="center">
          <template slot-scope="scope">
            <div>
              <p style="margin:0">{{scope.row.add_by}}</p>
              <p style="margin:0;font-size:12px;color:#999;">{{ addDateFormat(scope.row.addTime, 'YYYY-MM-DD') }}</p>
            </div>
          </template>
        </el-table-column>
        <!-- <el-table-column label="最终修改人" min-width="124" show-overflow-tooltip>
          <template slot-scope="scope">
            <div>
              <p style="margin:0">{{scope.row.modified_by}}</p>
              <p style="margin:0;font-size:12px">{{ addDateFormat(scope.row.modifiedTime) }}</p>
            </div>
          </template>
        </el-table-column> -->
        <!-- <el-table-column label="操作" width="132">
          <template slot-scope="scope">
            <el-tooltip content="编辑" placement="top" effect="light">
              <i class="gm-icon gm-icon-edit" @click="editDisplay(scope.row)" v-if="isAdminUser"></i>
            </el-tooltip>
            <el-tooltip content="查看商品" placement="top" effect="light">
              <i class="display-product-icon display-icon" @click="showProducts(scope.row), dialogTableVisible = true"></i>
            </el-tooltip>
            <el-tooltip content="删除" placement="top" effect="light">
              <i class="gm-icon gm-icon-delete" @click="deleteDisplay(scope.row)" v-if="isAdminUser"></i>
            </el-tooltip>
          </template>
        </el-table-column> -->
    ...
    <el-dialog
      ...
      @close="addAndImmportDialogVisiable = false">
        ...
        <div v-show="dialogShowBlockIndex === 0">
          ...
          <span slot="footer" class="dialog-footer">
            <el-button size="small" style="width:80px" @click="addAndImmportDialogVisiable = false">取消</el-button>
            <el-button type="primary" style="width:80px" size="small" @click="saveDisplays('displayInfoForm')" v-show="isAddButton">添加</el-button>
            <el-button type="primary" style="width:80px" size="small" @click="updateDisplays('displayInfoForm')" v-show="!isAddButton">保存</el-button>
          </span>
        </div>
...
import {displayTypeOptions, isPresellOptions, isDisplayOptions, latestDisplayTimeOptions, presellSelectOption} from '@/const/options'
...
export default {
  ...
  created () {
    ...
    let queryDataKey = Object.keys(this.$route.query)
  ...
  methods: {
    subscribeAddScene () {
      if (this.ctype === 'cooperative') {
        let _vm = this
        // 监听消息
        PubSub.subscribe('addScene', function(msg, data) {
          if (data) {
            _vm.queryDisplays()
          }
        })
    ...
    // 搜索场次
    searchDisplay () {
      let queryData = JSON.parse(JSON.stringify(this.searchDisplayForm))
      Object.keys(queryData).forEach(key => {
        if (queryData[key] === '' || queryData[key] === null) delete queryData[key]
      })
      // this.$router.push({path: '/data/display-info', query: queryData})
      if (this.ctype !== 'cooperative') {
        this.$router.push({path: '/operation-center/display-management', query: queryData})
      }
    ...
    exportStarSubmit: function () {
      let display_ids = JSON.parse(JSON.stringify(this.delDisplay))
      if (display_ids.length > 0) {
        let ptemp = {
          display_ids: JSON.stringify(display_ids)
        }
        let url = `${this.domain}/api/star/export_displays?${qs.stringify(ptemp)}&Authorization=${getToken()}`
        window.open(url)
      } else {
        let exportpass = {
          star_id: ''
        }
        this.allStars.forEach((item, index) => {
        ...
        let queryData = JSON.parse(JSON.stringify(displaypass))
        Object.keys(queryData).forEach(key => {
          if (queryData[key] === '' || queryData[key] === null) delete queryData[key]
        })
        exportpass = {...queryData, ...exportpass}
        if (this.ctype === 'cooperative') {
          let _CooperationDetail = this.$store.getters['cooperative/CooperationDetail']
          if (_CooperationDetail) {
            exportpass.cooperation_id = _CooperationDetail.cooperation_id
          }
        }
        if (exportpass.hasOwnProperty('num')) delete exportpass.num
        if (exportpass.hasOwnProperty('page_num')) delete exportpass.page_num
        let url = `${this.domain}/api/star/export_displays?${qs.stringify(exportpass)}&Authorization=${getToken()}`
        window.open(url)
      }
    },
    // 主播昵称输入的时候
    ...
    queryDisplays: function () {
      ...
      if (this.ctype === 'cooperative') {
        let CooperationDetail = this.$store.getters['cooperative/CooperationDetail']
        ...
        if (data.success) {
          ...
          for (var i in data.data.data) {
            this.displayData.push({
              ...
              starId: data.data.data[i].star_id,
              starName: data.data.data[i].star_name,
              displayType: data.data.data[i].display_type,
              pv: data.data.data[i].pv,
              displayTime: data.data.data[i].display_time,
              add_by: data.data.data[i].add_by,
              modified_by: data.data.data[i].modified_by,
              isPresell: data.data.data[i].is_presell,
              modifiedTime: data.data.data[i].gmt_modified,
              addTime: data.data.data[i].gmt_create,
              isDisplay: data.data.data[i].is_display,
              displaySalesAmount: data.data.data[i].display_sales_amount,
              displaySalesNum: data.data.data[i].display_sales_num,
              uv: data.data.data[i].uv,
              products_num: data.data.data[i].products_num,
              products_click_num: data.data.data[i].products_click_num
            })
          }
    ...
    showProducts: function (scope) {
      let data = {'displayId': scope.id, 'currentstarId': scope.starId, 'currentStarName': scope.starName}
      if (this.ctype === 'cooperative') {
        this.$refs.sceneDetailDialog.show(data)
      } else {
        this.$router.push({path: '/product-detail', query: data})
      }
    },
    displayTypeFormate,
    // categoryFormate,
      ...
      return (
        <span>总销售情况
          <el-tooltip placement="top" effect="light">
            <div slot="content">
              <p>未出单（有销售数据）：该销售额或销售量属于关联商品的情况下，显示【未出单】状态。</p>
...
<style>
  ...
  .add-and-immport-dialog .cutom-header span.active {
    background: #fff;
    color: #5C82FF!important;
    position: relative;
  }
  .add-and-immport-dialog .cutom-header span.active::after {
    content: " ";
    display: block;
    width: 100%;
    height: 2px;
  ...
  .add-and-immport-dialog .download-btns button:active {
