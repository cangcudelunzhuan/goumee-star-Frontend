      ...
      <div class="plan-info">
        <label>
          ...
          <el-input v-model="planName" placeholder="请输入方案名称" size="small">
            <span slot="suffix" v-show="planName === ''">列如：双十一阿玛尼口红直播方案</span>
          </el-input>
        </label>
        <label>
          <span>方案生成日期：<em>{{computDate}}</em></span>
        </label>
        <label>
          <span>方案生成人：<em>{{userName}}</em></span>
        </label>
      </div>
      <div class="anchor-list">
        <div class="list-title">已选主播：<span>{{anchors.length}}人</span></div>
        <div class="list-main">
      ...
      try {
        ...
        if (res.status === 200 && res.data.success) {
          ...
          this.$confirm('成功生成方案', '提示', {
            ...
            // 跳转需求详情页面
            this.$router.replace({
              path: `/brand/requirementDetail?requirementId=${this.lastPageSavedData.requirement.requirement_id}`
            })
            // 清空上个页面保存的数据
            sessionStorage.removeItem('anchors')
          })
