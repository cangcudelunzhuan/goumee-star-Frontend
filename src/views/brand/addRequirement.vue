  ...
  <div id="addRequirement">
    ...
    <div class="requirement-setting">
      ...
      <div class="setting-content">
        <el-form class="setting-form" :model="ruleForm" ref="ruleForm" :rules="rules" label-position="right" label-width="110px">
          <el-form-item label="品牌名称：" prop="ppname">
            <!-- <el-input v-model="ruleForm.ppname"></el-input> -->
            <el-select v-model="ruleForm.ppname" filterable placeholder="请选择" @change="brandChangeHandle" @blur="brandBlurHandle">
              <el-option
          ...
          <el-form-item label="需求名称：" prop="xqname">
            <el-input v-model="ruleForm.xqname" placeholder="请输入需求名称" @input="requirementNameTyping"></el-input>
          ...
          <div class="setting-footer">
            <el-button type="primary" @click="submitForm('ruleForm', 'next')">下一步匹配主播</el-button>
...
import { getBrandList, createRequirement, requirementDetail, updateRequirement, validateRequirementName } from '@/api/brand'
...
export default {
  ...
  methods: {
    ...
    async validateRequirementName (rule, value, callback) {
      if (!value) {
        ...
        if (this.requiementNameRepeatTipIsShowed) {
          ...
          let res = await validateRequirementName({
    ...
    async saveRequirement (type) {
      try {
        let res = await createRequirement({
        ...
        if (res.status === 200 && res.data.success) {
          if (type === 'next') {
            this.$router.push({
              path: `/brand/match-anchor?rid=${res.data.data}&from=createRequirement`
            })
          } else if (type === 'save') {
            this.$message({
              showClose: true,
    ...
    async getRequirementDetail (rid) {
      try {
        let res = await requirementDetail({
          requirement_id: rid
        })
    ...
    async modifyRequirementDetail (type) {
      try {
        let res = await updateRequirement({
        ...
        if (res.status === 200 && res.data.success) {
          if (type === 'next') {
            this.$router.push({
              path: `/brand/match-anchor?rid=${this.$route.query.rid}&from=createRequirement`
            })
          } else if (type === 'save') {
            this.$message({
              showClose: true,
