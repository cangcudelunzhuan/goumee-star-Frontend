<template>
  <section class="page-section-brand">
    <el-row class="search-requirement-row">
      <el-form class="search-bar" :inline="true" size="small">
        <el-form-item label="状态" class="media-cls">
          <el-select v-model="statusText" placeholder="请选择" style="width: 120px;" @change="changeSelect">
        ...
        <el-form-item>
          <el-button @click="resetRequirement">重置</el-button>
        </el-form-item>
      </el-form>
      <el-form v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.add_brand) && $store.getters['user/getUserRole'].includes(RIGHT_CODE.del_brand)" :inline="true" size="small" class="btn-bar">
        <el-form-item>
    ...
    <el-row style="box-sizing:border-box; z-index: 1;">
      <el-table :data="queryBrandTable" stripe :header-row-style="{fontSize:'14px'}" :header-cell-style="{background:'#f0f1f2',height:'50px',color:'#666'}" class="brand-requirement-table" v-loading="queryBrandTableLoading"
        :height="tableHeight + ''" @row-click="onRowClick" :row-style="{height: '60px'}" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="add_by" label="录入人"></el-table-column>
        <el-table-column prop="create_time_str" label="录入时间" show-overflow-tooltip align="center" min-width="95"></el-table-column>
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <div>
              <el-switch v-model="scope.row.status" @change="changeStatus(scope.row)" :disabled="!$store.getters['user/getUserRole'].includes(RIGHT_CODE.update_brand)"></el-switch>
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog class="dialog-anthor-list" :visible.sync="addBrandVisible" title="新增品牌" @close="addBrandVisible=false" top="10vh" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-row>
        <el-col :span="4" align="right" style="font-size: 16px; line-height: 32px;">品牌名称：</el-col>
        <el-col :span="20">
          <el-tag :key="tag" v-for="tag in dynamicTags" closable :disable-transitions="false" @close="handleClose(tag)">
            {{tag.value}}
          </el-tag>
          <el-input class="input-new-tag" v-if="inputVisible" v-model="inputValue" ref="saveTagInput" size="small" maxlength="12" @keyup.enter.native="handleInputConfirm" @blur="handleInputConfirm">
          </el-input>
...
export default {
  ...
  methods: {
    ...
    async clickQueryBrand() {
      ...
      let response = await queryBrand(params)
    ...
    setHeight() {
      if (document.querySelector('.search-requirement-row')) {
        this.tableHeight = window.innerHeight - 48 - 46 - document.querySelector('.search-requirement-row').clientHeight - 14 - 46 - 54 + 56
      }
    },
    // 重置条件
    resetRequirement() {
