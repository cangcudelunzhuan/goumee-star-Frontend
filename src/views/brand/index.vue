  ...
  <section class="requirement-index" style=" min-width: 972px;">
      ...
      <el-form class="search-bar2" :inline="true" size="small">
        <el-form class="search-bar2">
          <el-form-item label="提报日期：" class="media-cls">
            <el-date-picker v-model="times" unlink-panels type="daterange" size="small" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions1"
          ...
          <el-form-item label="品牌名称：">
            ...
            <el-select ref="brandNameEdit" size="small" class="text-input" v-model="brand_name" clearable filterable placeholder="输入品牌名">
              <el-option v-for="item in brandList" :key="item.id" :label="item.brand_name" :value="item.brand_name">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="需求名称：">
            <el-input placeholder="输入需求名" style="width: 260px" size="small" clearable v-model="requirement_name"></el-input>
          </el-form-item>
          <el-form-item label="提报人：">
            <el-input placeholder="输入提报人" style="width: 260px" size="small" clearable v-model="add_by"></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="clickQueryRequirement" size="small">查询</el-button>
          </el-form-item>
          <el-form-item>
            <el-button @click="resetRequirement" size="small">重置</el-button>
          </el-form-item>
        </el-form>
      </el-form>
    </el-row>
    <el-row v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.add_req) && $store.getters['user/getUserRole'].includes(RIGHT_CODE.del_req)" class="search-requirement-row" style="margin-top: 12px;">
    ...
    <el-row style="box-sizing:border-box; z-index: 1; padding:12px; background: #fff;">
      <el-table
        ...
        :header-cell-style="{background:'#f0f1f2',height:'50px',color:'#666'}"
          ...
          <template slot-scope="scope">
            <div class="edit-box">
              ...
              <i
                ...
                @click.stop="onEditClick(scope.row)"></i>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="plan_count" label="已生成方案数" align="center" width="150"></el-table-column>
        <el-table-column prop="report_date" label="提报日期" align="center" width="150"></el-table-column>
        <el-table-column prop="add_by" label="需求提报人" align="center" width="150"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async clickQueryRequirement() {
      ...
      let response = await queryRequirement(params)
    ...
    clickAddRequirement() {
      this.$router.push({ path: '/brand/addrequirement' })
    },
    goRequirementDetail(row) {
      this.$router.push({ path: '/brand/requirementDetail', query: { requirementId: row.id } })
    },
