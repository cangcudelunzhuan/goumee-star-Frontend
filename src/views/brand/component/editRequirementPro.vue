<template>
  <el-form
    ...
    <el-form-item label="商品名称" prop="product_name" :rules="[{required: true, message: '请输入商品名称'}]">
      <el-input size="small" name="product_name" v-model="importedProListItem.product_name"></el-input>
    </el-form-item>
    <el-form-item label="商品链接" prop="product_url" :rules="[{required: true, message: '请输入商品链接'}]">
      <el-input size="small" name="product_url" v-model="importedProListItem.product_url"></el-input>
    </el-form-item>
    ...
    <el-form-item label="活动价(客单价)">
      <el-input size="small" v-model="importedProListItem.product_sales_price" type="number">
        <template slot="append">元</template>
      </el-input>
    </el-form-item>
  </el-form>
...
export default {
  ...
  mounted () {
    this.$emit('passRefFormName', this.$refs['importedProListItem'])
  },
  methods: {
  }
}
