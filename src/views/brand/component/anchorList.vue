  ...
  <section class="anchor-list-wrapper">
    <div class="layout-table">
      <div class="layout-head">
        ...
        <div class="head-right">注：<i class="gm-icon-percent"></i><span>图标表示最近15款平均销售额（元）/&nbsp;最近15款点击数（次）</span></div>
      </div>
          ...
          <el-scrollbar style="width: 100%;" ref="scroll">
            <div class="body-right-box">
              <template v-for="(item, rangeIndex) in costRangeList">
                <div class="item" v-if="item.star_list.length>0" :key="rangeIndex" :style="'width: '+ getMaxWidth() +'px'">
                  <template>
                    <div class="anchor-item-wrapper" v-for="(anchor, anchorIndex) in item.star_list" :key="anchorIndex" @click="selectAnchor(rangeIndex, anchorIndex)">
                      <anchor-item :anchor="anchor" :rangeIndex="rangeIndex" :anchorIndex="anchorIndex" @del="delStar(anchor, rangeIndex, anchorIndex)"></anchor-item>
                    </div>
                  </template>
                </div>
              </template>
...
export default {
  ...
  methods: {
    getMaxWidth() {
      let _width = 0
      let _len = 0
      this.costRangeList.forEach((item, index) => {
