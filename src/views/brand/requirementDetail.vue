<template>
  <section class="child-container requirement-detail-container">
    <el-row class="detail-text-box">
      ...
      <div class="text-name"><span v-show="!requirementNameEdit" @click="$router.back()">{{requirement_name}}</span>
        <el-input
          ...
          @blur="editRequirement"></el-input>
        <i v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.update_req)" class="edit-btn el-icon-edit" @click="setRequirementNameEdit"></i>
      </div>
      <div class="text-info">
        <span>品牌名称：<b v-show="!brandNameEdit">{{brand_name}}</b>
          ...
          <el-select ref="brandNameEdit" :class="{nobrandNameEdit:!brandNameEdit}" size="mini" class="text-input" v-model="brand_name" filterable placeholder="请选择" @change="brandChange" @blur="brandBlur">
            <el-option v-for="item in brandList" :key="item.id" :label="item.brand_name" :value="item.brand_name">
            </el-option>
          </el-select>
          <i v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.update_req)" class="edit-btn el-icon-edit" @click="setBrandNameEdit"></i>
        </span>
        <span>预算：<b v-show="!budgetEdit">¥{{budget}}</b>
          <el-input ref="budgetEdit" v-show="budgetEdit" size="mini" class="w90" v-model="budget" maxlength="9" @keyup.enter.native="editRequirement" @blur="editRequirement"></el-input>
          <i v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.update_req)" class="edit-btn el-icon-edit" @click="setBudgetEdit"></i>
        </span>
        <span>提报日期：<b>{{report_date}}</b></span>
      </div>
      <i class="el-icon-arrow-left" @click="$router.back()"></i>
    </el-row>
    <el-row class="detail-table-wrapper">
      <el-row class="btn-wrapper">
        <el-button v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.add_req)" type="primary" size="small" @click="goPlan">新增方案</el-button>
        <el-button v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.view_req_list)" size="small" @click="output">批量导出</el-button>
      </el-row>
      <el-table :data="plans" stripe :header-row-style="{fontSize:'14px'}" :header-cell-style="{background:'#f0f1f2',height:'50px',color:'#666'}" class="detail-table" :row-style="{height: '60px'}"
        @selection-change="handleSelectionChange" :height="tableHeight + ''">
        <el-table-column type="selection" width="50" :selectable="row => row.status === 1"></el-table-column>
          ...
          <template slot-scope="scope">
            <div class="edit-box">
              ...
              <el-input
                ...
                @blur="onInputBlur(scope.row)"></el-input>
              <i
                ...
                @click.stop="onEditClick(scope.row.plan_id)"></i>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="star_count" label="已选主播/个" align="center"></el-table-column>
        <el-table-column prop="create_date" label="方案生成日期" align="center" min-width="95"></el-table-column>
        <el-table-column prop="create_by" label="方案生成人" align="center"></el-table-column>
        <el-table-column label="生成状态">
          <template slot-scope="scope">
            <div class="icon-status status1" v-if="scope.row.status">
              已生成
            </div>
            <div class="icon-status status2" v-else>
              生成中
            </div>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <div>
              <el-tooltip class="item" effect="light" content="匹配主播" placement="top" v-if="!scope.row.status">
                <i v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.update_req)" class="icon-match" @click="goPlan(scope.row)"></i>
              </el-tooltip>
              <el-tooltip class="item" effect="light" content="查看主播" placement="top" v-if="scope.row.status">
                <i class="icon-look" @click="viewSelectedStars(scope.row)"></i>
              </el-tooltip>
              <el-tooltip class="item" effect="light" content="删除方案" placement="top">
                <i v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.del_req)" class="el-icon-delete" @click="handleDeletePlan(scope.row)"></i>
              </el-tooltip>
            </div>
          </template>
        </el-table-column>
      ...
      <div class="pagination-wrapper">
        <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="pagination.currentPage" :page-size.sync="pagination.pageSize"
          :page-sizes.sync="pagination.pageSizes" layout="prev, pager, next, jumper" :total="pagination.total">
        </el-pagination>
      </div>
    </el-row>
    <el-dialog class="dialog-star-list" width="890px" :visible.sync="lookStarVisible" @close="lookStarVisible=false" title="查看主播" top="10vh" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-row>
        <div class="star-tips">已选主播：<b>{{starList.length}}人</b></div>
        <ul class="anchor-list-wrapper">
          <li class="add-anchor anchor-item" @click="continueAddAnchor">
            <i class="el-icon-circle-plus-outline"></i>
            <p>添加主播</p>
          </li>
          <li class="anchor-item" v-for="(anchor, index) in starList" :key="index" @click="goStar(anchor)">
            ...
            <div class="delete-mask" v-show="deleteMaskShowId === anchor.id">
              ...
              <el-button type="danger" size="mini" @click.stop="deletePlanAnchor(anchor.id)" style="background: #f26467;">删除</el-button>
            </div>
          </li>
        </ul>
      </el-row>
...
<script>
import { requirementDetail, updateRequirement, queryBrand, deleteRequirementPlan, viewSelectedStars, updateRequirementPlanName, deletePlanAnchor } from '@/api/brand'
...
export default {
  ...
  methods: {
    ...
    goPlan(row) {
      ...
      this.$router.push({ path: '/brand/match-anchor', query })
    },
    ...
    output () {
      ...
      let ids = this.selectList.map(item => item.plan_id)
      let token = getToken()
      window.open(`${process.env.BASE_API}/api/brand2/export_plan?ids=${JSON.stringify(ids)}&Authorization=${token}`)
    },
    // 删除方案中的主播
