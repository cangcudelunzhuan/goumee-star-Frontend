<template>
  <section class="page-section">
    <el-button size="small" icon="el-icon-arrow-left" type="primary" style="margin-bottom:14px; font-size: 14px; padding: 8px 15px;" @click="$router.push({path: 'brand-requirement'})">返回</el-button>
    <!-- requiremet form -edit/add -->
    <transition name="demand-active">
      <el-row style="background:#fff;" v-show="!isShowReqDetail">
        <p style="margin-left:16px;line-height:36px;color:#333;font-size:15px">编辑需求</p>
        <el-form v-loading="addReqLoading" size="small" :inline="true" :model="requirementInfoForm" class="add-demand-form" ref="requirementInfoForm">
          <el-row>
            <el-form-item label="品牌名称" prop="brand_name" :rules="[{required: true, message: '品牌名称不能为空'}]">
              <el-select placeholder="请选择品牌名称" name="brand_name" v-model="requirementInfoForm.brand_name" filterable clearable>
                <el-option v-for="item in brandList" :key="item.id" :value="item.brand_name" :label="item.brand_name"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="利润率" prop="profit" :rules="[{required: true, message: '利润率不能为空'}]">
              <el-input placeholder="请输入内容" name="profit" v-model="requirementInfoForm.profit" type="number">
                <template slot="append"> % </template>
              </el-input>
            </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="需求名称" prop="requirement_name" :rules="[{required: true, message: '需求名称不能为空'}]">
              <el-input placeholder="请输入内容" name="requirement_name" v-model="requirementInfoForm.requirement_name"></el-input>
            </el-form-item>
            <el-form-item label="预 算" prop="budget" :rules="[{required: true, message: '预算不能为空'}]">
              <el-input placeholder="请输入内容" name="budget" v-model="requirementInfoForm.budget" type="number">
                <template slot="append">元</template>
              </el-input>
            </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="直播类型" prop="display_type" :rules="[{required: true, message: '直播类型不能为空'}]">
              ...
              <el-select v-model="requirementInfoForm.display_type" name="display_type">
                <el-option v-for="item in displayTypeOptions" :value="item.value" :label="item.label" :key="item.value"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="返 点" prop="rebate" :rules="[{required: true, message: '返点不能为空'}]">
              <el-input placeholder="请输入内容" name="rebate" type="number" v-model="requirementInfoForm.rebate">
                <template slot="append">元</template>
              </el-input>
            </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="提报日期">
              <el-date-picker placeholder="请选择提报日期" v-model="requirementInfoForm.report_date" value-format="yyyy-MM-dd" format="yyyy-MM-dd">
              </el-date-picker>
            </el-form-item>
            <el-form-item label="KPI指标" :prop="'[' + kpiOptions + ']'" :rules="[{required: true, message: 'KPI指标不能为空'}]" class="kpi-target">
              <el-input type="number" placeholder="请输入内容" class="input-with-select" :name="[kpiOptions]" v-model="requirementInfoForm[kpiOptions]" :maxlength="9">
                <el-select size="small" placeholder="请选择" slot="prepend" v-model="kpiOptions">
                  <el-option label="销售额/元" value="kpi_sales_amount"></el-option>
                  <!-- <el-option label="UV/次" value="kpi_uv"></el-option> -->
                </el-select>
              </el-input>
            </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="需求介绍">
              <el-input placeholder="请输入内容" type="textarea" v-model="requirementInfoForm.introduce" :autosize="{ minRows: 1, maxRows: 3}"></el-input>
            </el-form-item>
          </el-row>
        </el-form>
        <el-row style="margin:16px 0;text-align:center">
          <el-button size="medium" type="primary" @click="isShowReqDetail = true" v-show="isUpdateReqBtn">取消保存</el-button>
          <el-button size="medium" type="primary" @click="clickUpdateRequirement('requirementInfoForm')" v-show="isUpdateReqBtn">
            保存需求
          </el-button>
          <el-button size="medium" type="primary" @click="clickSaveRequirement('requirementInfoForm')" v-show="!isUpdateReqBtn">
            新增需求
          </el-button>
        </el-row>
      </el-row>
    ...
    <transition>
      <el-row v-show="isShowReqDetail" v-loading="reqDetailLoading" class="demand-detail-el-row">
        ...
        <requirementDetail :requirementDetailInfo="requirementDetailInfo" :brandList="brandList"></requirementDetail>
        <el-row class="demand-detail-footer">
          <p>剩余成本：<span>{{last_cost}}元</span></p>
          <p>总成本：<span>{{requirementDetailInfo.total_cost}}元</span></p>
        </el-row>
      </el-row>
    </transition>
    <!-- button-row -->
    <transition name="demand-active">
      <el-row class="row-button" v-show="isShowReqDetail || isImportedPro">
        <el-upload class="row-button-upload" ref="upload" :action='""' :http-request="clickImportProduct" :show-file-list="false" multiple :file-list="fileList">
    ...
    <el-row class="row-table" v-show="isShowReqDetail || isImportedPro">
      <el-table class="require-table" :data="list" stripe :header-cell-style="{background: '#f0f1f2',height:'48px',color:'#666'}" empty-text="请先导入商品" :row-key="getRowKeys" :expand-row-keys="expands" v-loading="tableLoading">
        <el-table-column type="expand" width="20">
          <template slot-scope="scope">
            <el-scrollbar style="width: 100%; overflow-y: hidden;" ref="scroll">
              <div class="list-box" :style="'width:' + scope.row.chosen_plan_list.length * 232 + 'px'">
                <div class="list-item-wrapper" v-for="(anchor, index) in scope.row.chosen_plan_list" :key="index">
                  <anchor-item :anchor="anchor" :isChose="1" @del="updateStar({method: 'del', anchor, index})" @update="updateStar"></anchor-item>
                </div>
              </div>
            </el-scrollbar>
          </template>
        ...
        <el-table-column label="匹配状态" min-width="140">
          <template slot-scope="scope">
            <span>
              ...
              <template v-else>
                <span v-if="scope.row.chosen_plan_list.length > 0" class="type-anchor" :class="{active: expands.length>0&&expands[0]===scope.row.id}" @click="showStarList(scope)">主播已确定</span>
        ...
        <el-table-column label="操作" min-width="240">
          <template slot-scope="scope">
            <el-button size="mini" type="primary" @click="clickChooseAnchor(scope)"><template v-if="scope.row.chosen_plan_list.length > 0">继续选择</template><template v-else>选择主播</template></el-button>
            <el-button size="mini" type="primary" @click="clickEditProduct(scope.row)">编辑</el-button>
            <el-button size="mini" type="primary" @click="deleteProduct(scope)">删除<span v-show="false">{{scope.row}}</span></el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
    <el-dialog :visible.sync="editProDialogVisible" width="34%" title="编辑商品信息" class="demand-edit-pro-dialog" @close="closeEditReqProDialog" :close-on-click-modal="false" :close-on-press-escape="false">
      <editRequirementPro :importedProListItem="importedProListItem" @passRefFormName="getRefFormName" ref="importedProListItem">
      </editRequirementPro>
      <span slot="footer">
        <el-row>
    ...
    <el-dialog class="dialog-anthor-list" :visible.sync="chooseAnchorVisible" title="候选集列表" @close="cancelStarList" top="10vh" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-row type="flex">
        <el-col class="dialog-anthor-text"><span>总成本：</span><b class="total">{{requirementDetailInfo.total_cost}}元</b><span>剩余成本：</span><b class="last">{{last_cost}}元</b></el-col>
        <el-col :span="8">
          <el-select placeholder="输入主播名称手动添加主播" v-model="currentStar" size="small" multiple-limit="1" popper-class="dialog-select-class" filterable clearable style="width: 100%;">
            <el-option v-for="(item, index) in allStars" :value="item.value" :key="index" :label="item.value">
              <span class="dialog-anthor-select">{{item.value}}
                <i class="tips" v-if="checkIsAdded(item.star_id)">(已添加)</i>
              </span>
              <i class="dialog-anthor-btn-add" v-if="!checkIsAdded(item.star_id)" @click="selectOption(item)">添加</i>
            </el-option>
          </el-select>
        </el-col>
      </el-row>
      ...
      <div slot="footer" class="dialog-footer">
        <span class="text">已选主播<b>{{selectCount}}</b>位</span>
...
import { queryBrand, saveRequirment, updateRequirement, queryRequirement, queryRequirementProduct, delRequirementProduct, updateRequirementProduct } from '@/api/brand'
  ...
  methods: {
    clickSaveRequirement(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          let requirementpass = this.requirementInfoForm
          ...
          saveRequirment(requirementpass).then(response => {
            ...
            if (result.success) {
              ...
              this.isUpdateReqBtn = true
              this.isShowReqDetail = true
              this.$router.push({ path: '/requirement-info', query: { requirementId: this.currentId } })
              this.idQueryRequirement()
              this.idQueryReqmentPro()
              for (var item in this.requirementInfoForm) {
    ...
    clickUpdateRequirement(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.addReqLoading = true
          let requirementpass = JSON.parse(JSON.stringify(this.requirementInfoForm))
          let deleteArr = ['add_by', 'brand_id', 'flag', 'gmt_create', 'gmt_modified', 'modified_by', 'total_cost', 'status']
    ...
    // 查询候选集
    idQueryRequirement() {
      let requirementpass = { requirement_id: '' }
      if (this.isUpdateReqBtn) {
        ...
        queryRequirement(requirementpass)
          .then(response => {
            ...
            this.$store.commit('requirement/setDisplayType', this.requirementDetailInfo.display_type)
    ...
    editRequirementDetail(formName) {
      // 点击编辑需求详情
      this.$confirm('修改需求可能会导致已经匹配的候选集不可用！', '提示', {
        ...
        .then(() => {
          if (this.requirementDetailInfo.status !== 0) {
            ...
            this.requirementInfoForm.report_date = this.showProDateFormat(this.requirementInfoForm.report_date)
          }
        })
        .catch(() => {})
    },
    ...
    saveEditedProduct() {
      ...
      this.comEditProValidate.validate(valid => {
        // this.$refs[formName].validate(valid => {
        if (valid) {
          let deleteArr = ['add_by', 'flag', 'gmt_create', 'gmt_modified', 'is_match_candidate', 'modified_by', 'requirement_id', 'shop_name', 'chosen_plan_list', 'cost_range_list', 'star_num', 'used_cost', 'display_cost', 'require_product_id']
          deleteArr.forEach(key => {
    ...
    // addMatchedSet(val) {
    //   this.$router.push({ path: '/match-info', query: { requirementId: this.currentId, index: val } })
    // },
    clickProducePlan() {
      this.$router.push({ path: '/plan-info', query: { requirementId: this.currentId } })
    },
    // 选择主播
    ...
    clickQueryStar() {
      ...
      queryStars({ star_name: this.currentStar }).then(response => {
        let result = response.data
        if (result.success) {
          let star1 = result.data.data[0]
          let _index = this.$store.state.requirement.currentChooseAnchorIndex
          let params = {
            display_num: 1,
            is_chosen: '0',
          ...
          addCandidate(params).then(res => {
            ...
            if (res.success) {
              let star2 = res.data
              let star = Object.assign(star1, star2)
