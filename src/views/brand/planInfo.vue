<template>
  <section class="page-section">
    <el-row>
      <el-button size="small" icon="el-icon-arrow-left" type="primary" @click="$router.go(-1)">返回</el-button>
    </el-row>
    <el-row class="top-tabs">
      <i class="el-icon-arrow-right" @click="isRightArrow = !isRightArrow" :class="isRightArrow ? 'is-right':'is-bot'"></i>
        ...
        <el-tab-pane label="需求详情" name="requirement">
          <transition name="plan-active">
            <el-row v-show="!isRightArrow" v-loading="requirementDetailLoading">
              <requirement-detail :requirementDetailInfo="requirementDetailInfo"></requirement-detail>
              <el-col class="detail-footer">成本计算：<span>{{totalCost}} 元</span></el-col>
            </el-row>
          </transition>
        </el-tab-pane>
        <el-tab-pane label="商品列表" name="product">
          <transition name="plan-active">
            <el-row v-show="!isRightArrow" style="padding: 0 10px" v-loading="productListLoading">
              <el-row class="product-list-desc" v-for="(item, index) in importedProInfo" :key="index">
                <el-col :span="2"><i class="icon-num">{{index + 1}}</i></el-col>
                <el-col :span="22">
                  ...
                  <el-row :gutter="5" class="pro-desc">
                    ...
                    <el-col :span="7">活动价(客单价)：<span>{{item.product_sales_price}} 元</span></el-col>
    ...
    <el-row class="plan-list" v-if="matchPlanList.length === 0">
      <p style="text-align:center;color:#888;line-height:120px">还没有方案~</p>
    </el-row>
    <template v-if="matchPlanList.length > 0">
      <el-row class="plan-list" v-for="(item, index) in matchPlanList" :key="index" v-loading="planListLoading">
        <el-col :span="5" class="plan-list-desc-star">
          ...
          <el-row class="replace-star">
            <el-button size="small" type="primary" @click="clickReplaceStar(index);replaceStarDialogVisible = true">替换主播</el-button>
          </el-row>
        </el-col>
        <el-col :span="19" class="plan-list-desc-pro">
          <el-row class="pro-top">
            <div class="pro-info-par gm-scrollBar-style">
              <el-row class="pro-info" v-for="(arg, key) in item.not_scheduled_require_product_data"
                :key="key">
                ...
                <el-col :span="20" class="pro-info-desc text-overflow-cls">
                  <div class="text-overflow-cls">
                    {{arg.product_name}}
                    <span style="color:#999">直播成本：{{arg.display_cost}} 元</span>
                  </div>
                </el-col>
              </el-row>
            </div>
            <el-row class="pro-info" v-show="item.not_scheduled_require_product_data.length === 0">
              ...
              <el-col :span="20" style="line-height:30px;color:#666">
                暂无未排期商品~
              </el-col>
            </el-row>
            <el-row class="date-info">
              ...
              <el-col :span="20" class="date-info-desc">
                <span style="line-height:30px;color:#666" v-if="matchPlanList[index].schedule_list.length === 0">暂无排期~</span>
                  ...
                  <div v-for="(tag, key) in item.schedule_list" :key="key">
                    <span class="date-tag-span text-overflow-cls">
                      ...
                      <em :class="tag.is_display === 2 ? 'is-display':'not-display'">
                        {{getIsDisplay(tag)}} </em>
                      <i class="el-icon-circle-close" v-show="true" @click="deleteTheDate(tag.id)"></i>
                    </span>
                  </div>
                </div>
              </el-col>
          ...
          <el-row class="pro-bot">
            ...
            <el-col :span="4" class="add-schedule">
              <el-button size="small" type="primary" @click="clickAddSchedule(item, index)">添加排期</el-button>
            </el-col>
          </el-row>
        </el-col>
      </el-row>
    ...
    <!-- 替换主播弹框 -->
    <el-dialog title="替换主播" :visible.sync="replaceStarDialogVisible" :close-on-click-modal="false"
      :close-on-press-escape="false" @close="closeReplaceStarDialog" width="78%" class="replace-star-dialog">
      <el-row class="replace-star-dialog-header">
        <span>主播信息：</span>
        <el-select v-model="searchStarName" placeholder="请选择主播" size="small" filterable clearable>
          <el-option v-for="(item, index) in allStars" :key="index" :value="item.value" :label="item.label"
      ...
      <span slot="footer">
        <el-button size="small" style="width:72px" @click="replaceStarDialogVisible = false">取消</el-button>
        <el-button size="small" type="primary" @click="confirmReplace">确定替换</el-button>
      </span>
    </el-dialog>
    <!-- 添加排期弹框 -->
    <el-dialog title="主播排期" @close="closeAddScheduleDialog('addScheduleForm')" :visible.sync="addDateDialogVisible"
      class="add-date-dialog" width="30%" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-form size="small" :inline="true" label-position="top" label-width="110px" :model="addScheduleForm"
        ref="addScheduleForm">
        <el-form-item label="选择商品" prop="productList" :rules="[{required: true, message: '请选择至少一个商品'}]">
          <el-select placeholder="请选择要直播的商品" popper-class="date-product-option" clearable
            filterable multiple name="product_list" v-model="addScheduleForm.productList">
            <el-option :value="indexs" :label="pro.productName" v-for="(pro, indexs) in notScheduleProductList"
              :key="indexs"></el-option>
          </el-select>
          <div class="tag-checked-product" v-show="addScheduleForm.productList.length > 0">
            <span v-for="(item, index) in addScheduleForm.productList" :key="index" class="text-overflow-cls">
        ...
        <el-form-item label="直播日期" prop="displayTime" :rules="[{required: true, message: '直播日期是必填项'}]">
          <el-date-picker name="display_time" placeholder="请选择直播日期" value-format="yyyy-MM-dd"
            v-model="addScheduleForm.displayTime">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="是否预售" prop="isPresell" :rules="[{required: true, message: '请选择是否为预售场'}]">
          <el-select v-model="addScheduleForm.isPresell" name="isPresell">
            <el-option v-for="item in presellSelectOption" :key="item.value" :value="item.value"
              :label="item.label"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <div slot="footer">
        <el-button size="small" style="width:80px;margin-top:12px" @click="addDateDialogVisible = false">取消</el-button>
        <el-button size="small" type="primary" style="width:80px" @click="addSchedule('addScheduleForm')">添加</el-button>
      </div>
    </el-dialog>
    <!-- 排期详情弹框 -->
    <el-dialog title="排期详情" :visible.sync="dateDetailDialogVisible" width="80%"
      :close-on-click-modal="false" :close-on-press-escape="false" class="date-detail-dialog">
      <el-row class="date-detail-dialog-header">
        <span class="date-detail-tit">排期详情</span>
        <el-row>直播日期： <span>{{showProDateFormat(scheduleDetailDisplayTime)}}</span></el-row>
        <el-row>直播商品：
  ...
  methods: {
    ...
    addSchedule(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let proId = this.addScheduleForm.productList.map((item, index) => {
