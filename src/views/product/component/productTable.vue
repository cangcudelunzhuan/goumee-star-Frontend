  ...
  <section>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <div class="pro-info">
            <p v-show="scope.row.shop_name !== ''"><span class="w52">店&#12288;铺：</span><span>{{scope.row.shop_name}}<b v-if="scope.row.category !== 0"> - {{categoryFormate(scope.row)}}</b></span></p>
            <p v-show="scope.row.shop_name === ''"><span class="w52">品&#12288;类：</span><span>{{categoryFormate(scope.row)}}</span></p>
            <p style="white-space:nowrap;font-size:13px;line-height:18px;text-overflow:ellipsis;overflow: hidden;">
              ...
              <span>
                ...
                >
                  ...
                  <span slot="reference">
                    <!-- <span>{{ getTitle(scope.row.product_name) }}</span> -->
                    <span>{{ scope.row.product_name }}</span>
                  </span>
                </el-popover>
              </span>
            </p>
            <p><span class="w52">活动价：</span><span>{{scope.row.product_sales_price === -1 ? '未出单':scope.row.product_sales_price + '元'}}</span></p>
            <p class="pro-tag">
              <span
              ...
              <em :class="scope.row.is_presell === 1 || scope.row.is_presell === 2 ? 'is-presell-pro':'not-display-pro'">{{ getIsPresell(scope.row) }}</em>
              <em :class="scope.row.is_display === 2 ? 'is-display-pro':'not-display-pro'">{{ getIsDisplay(scope.row) }}</em>
              <em v-show="scope.row.is_related_product === 2">{{ scope.row.is_related_product === 2 ? '关联商品':'' }}</em>
            </p>
          </div>
        </template>
      </el-table-column>
      <el-table-column
        :render-header="customColumnHeaderSales"
        width="180"
      >
        <template slot-scope="scope">
          <div class="sales-desc">
            <p>
              <span>点击数：</span><span>{{scope.row.product_click_num === -1 ? '--' :( scope.row.product_click_num + ' 次')}}</span>
            </p>
            <p>
              <span>销售额：</span><span>{{scope.row.product_sales_amount === -1 ? '--' : (scope.row.product_sales_amount + ' 元')}}</span>
            </p>
            <p>
              <span>销售量：</span><span>{{scope.row.product_sales_num === -1 ? '--' : (scope.row.product_sales_num + ' 件')}}</span>
            </p>
            <p v-show="scope.row.product_click_num  === -1 && scope.row.product_sales_amount === -1 && scope.row.product_sales_num === -1">未出单</p>
          </div>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <div class="live-info">
            ...
            >
              <p style="margin:0">
                <span class="w70">直播标题：</span><span>{{scope.row.title}}</span>
              </p>
              <div slot="reference">
                <p v-show="scope.row.title !== ''">
                  <span class="w70">直播标题：</span><span>{{ getTitle(scope.row.title) }}</span>
                </p>
              </div>
            </el-popover>
            <p><span class="w70">主播昵称：</span><span>{{scope.row.star_name}}</span></p>
            <p><span class="w70">主播&#12288;ID：</span><span>{{scope.row.star_id}}</span></p>
            <!-- <p>
      ...
      >
        <template slot-scope="scope">
          <div class="live-time">
            <p><span>直播日期：</span><span>{{ showProDateFormat(scope.row.display_time) }}</span></p>
            <p><span>直播时间：</span><span>{{ getDisplayPeriod(scope.row) || '--' }}</span></p>
            <p><span>直播时长：</span><span>{{ getDuration(scope.row) }}</span></p>
          </div>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <div class="pro-info">
            <p style="color:#666">{{scope.row.add_by}}</p>
            <p style="font-size:12px;">{{addDateFormat(scope.row.gmt_create, 'YYYY-MM-DD')}}</p>
            <p style="color:#666">{{scope.row.modified_by}}</p>
            <p style="font-size:12px;">{{addDateFormat(scope.row.gmt_modified, 'YYYY-MM-DD')}}</p>
          </div>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <el-tooltip
            ...
            content="编辑"
          >
            <p v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.update_product)"><i
                class="gm-icon gm-icon-edit"
                @click="openEditProDialog(scope.row)"
              ></i></p>
          </el-tooltip>
          <el-tooltip
            placement="left"
            ...
            content="删除"
          >
            <p v-if="$store.getters['user/getUserRole'].includes(RIGHT_CODE.del_product)"><i
    ...
    >
      ...
      >
        <el-row>
          <el-form-item label="主播昵称">
            <el-input
              disabled
              v-model="proDialogFormData.star_name"
            ></el-input>
          </el-form-item>
          <el-form-item
            label="店铺名称"
        ...
        <el-row>
          ...
          >
            <el-select
              name="display_period"
              v-model="proDialogFormData.display_period"
            >
              <el-option
        ...
        <el-row>
          ...
          >
            <el-select
              v-model="proDialogFormData.category"
              required
            >
              <el-option
        ...
        <el-row>
          ...
          >
            <el-select
              name="duration"
              v-model="proDialogFormData.duration"
            >
              <el-option
        ...
        <el-row>
          <el-form-item class="checkbox-par">
            <el-checkbox
              v-model="proDialogFormData.is_top"
              :true-label="1"
              :false-label="0"
            >是否置顶</el-checkbox>
            <el-checkbox
      ...
      >
        <el-button
          size="small"
          @click="editProDialogVisible = false"
        >取消</el-button>
        <el-button
          size="small"
          type="primary"
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange (val) {
      this.$emit("passDeleteData", val)
    },
    openEditProDialog (row) {
      this.editProDialogVisible = true
      this.proDialogFormData = JSON.parse(JSON.stringify(row))
      if (this.proDialogFormData.category === 0) {
        this.proDialogFormData.category = "无"
      }
      for (var item in this.proDialogFormData) {
        if (this.proDialogFormData[item] === -1) {
          this.proDialogFormData[item] = ""
        }
      }
    },
    deleteProduct (row) {
    ...
    editProduct (formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let updateProductpass = JSON.parse(
            JSON.stringify(this.proDialogFormData)
          )
          updateProductpass.display_time = new Date(
            updateProductpass.display_time
          )
          let deleteArr = [
            "add_by",
            "flag",
          ...
          if (updateProductpass.category === "无") {
            ...
            updateProduct(updateProductpass).then(response => {
              ...
              if (result.success) {
                ...
                this.$gmMessage(result.message)
                this.$emit("comQueryProduct") // 查询商品
              } else {
                this.$gmMessage(result.message, "tip")
              }
            })
    ...
    customColumnHeaderSales (h, { column, $index }) {
      return (
        <span> 销售情况
          <el-tooltip
            content="未出单商品则不填销售额和销量字段,若填写则认为商品已出单"
            placement="top"
            effect="light"
          >
