...
export function exportCompany(params) {
  let token = getToken()
  window.open(
    `${process.env.BASE_API}/api/medium/export_company?ids=${
      params.ids
    }&Authorization=${token}`
  )
}
...
export function exportKol (params) {
  ...
  window.open(`${process.env.BASE_API}/api/kol/export_kols?${str}&Authorization=${token}`)
}
...
// 查询所有KOL名称
export function queryAllKolNames () {
  return request({
